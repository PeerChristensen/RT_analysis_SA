...
df %>% summarySE(measurevar="accuracy",groupvars=c("language","condition","congruent"))
...
#overview
react = df %>% 
  group_by(language,condition,congruent,sound) %>%
  dplyr::summarise(n=n(),
                   median = median(RT),
                   mean = mean(RT),
                   sd = sd(RT),
...
soundReact = df %>%
  group_by(language,sound,condition) %>%
  dplyr::summarise(n=n(),
                   median = median(RT),
                   mean = mean(RT),
                   sd = sd(RT),
...
df %>% summarySE(measurevar="RT",groupvars=c("language","condition","congruent"))
...
RTs2 = df %>% group_by(language,condition,block,sound) %>% dplyr::summarise(mean = mean(RT))
...
# distributions
df %>% ggplot(aes(x=RT, fill=language)) +
...
m1_a=lmer(RT~sound+(1|participant) + (1|item),data=df)
...
m2_a = lmer(RT~language+sound+condition+(1|participant),data=df) 
summary(m2_a)
...
m4 = lmer(RT~language*sound*congruent*condition+(1|participant),data=df)
summary(m4)
...
df %>% summarySE(measurevar="RT",groupvars=c("language","condition","congruent")) %>%
  ggplot() +
  aes(x = condition, y = RT, color = congruent) +
  geom_line(aes(group = congruent),size=1,position = dodge) +
...
df %>% summarySE(measurevar="RT",groupvars=c("language","condition","sound")) %>%
  ggplot() +
...
# per participant distribution by condition
grid.arrange(
df %>% 
  filter(language == "Af") %>% 
  ggplot(aes(x = RT, y = reorder(participant,RT),fill=condition)) +
...
df %>% 
  ...
  scale_x_continuous(expand = c(0.01, 0)) +
  theme_apa(legend.use.title =T) +
  scale_fill_viridis(discrete=T,begin=.1,end=1, alpha=.6), ncol = 2)
  
#by block - sound
df %>% 
  ggplot(aes(x = RT, y = reorder(block,-as.numeric(block)),fill=sound)) +
  geom_density_ridges(rel_min_height = 0.005,size=0.2) +
  scale_y_discrete(expand = c(0.01, 0)) +
  scale_x_continuous(expand = c(0.01, 0)) +
  theme_apa(legend.use.title =T) +
